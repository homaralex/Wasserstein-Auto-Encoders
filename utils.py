...
def load_disentanglement_data_dsprites():
    ...
    latents_sizes = metadata[
        b'latents_sizes']  # for some reason, I was getting a key error here so hard coded in the next line.
...
def load_data(model, seed=None):
    ...
    if dataset == 'dsprites':
        try:
            ...
            print(
                "Dataset file does not exist. You can download it here: https://github.com/deepmind/dsprites-dataset/ Save dsprites_ndarray_....npz in datasets folder as dsprites.npz")
        ...
        try:
            ...
            print(
                "Dataset file does not exist. You can download it here: https://www.dropbox.com/sh/flu98x7xghw2i59/AAC-eDY7TS9V54AxCtvBjGTAa?dl=0 Save celebA.npy in datasets folder")
        ...
        try:
            ...
            print(
                "Dataset file does not exist. You can download it here: https://www.dropbox.com/sh/flu98x7xghw2i59/AAC-eDY7TS9V54AxCtvBjGTAa?dl=0 Save celebA_mini.npy in datasets folder")
        ...
        try:
            ...
            print(
                "Dataset file does not exist. You can download it here: https://www.dropbox.com/sh/flu98x7xghw2i59/AAC-eDY7TS9V54AxCtvBjGTAa?dl=0 Save fading_squares.npy in datasets folder")
...
def plot_all_init(model):
    ...
    with tf.variable_scope("least_gaussian_2d_subspace"):
        ...
        projloss += tf.abs(tf.reduce_sum(tf.square(tf.reduce_mean(covhat_prior_samples_proj, 0))) - tf.reduce_sum(
            tf.square(tf.reduce_mean(covhat_real_embeddings, 0))))
        # We are maximizing
        projloss = -projloss
...
def plot_all(model, filename_appendage):
    ...
    fig.suptitle("Cols: {Test, Train, Prior samples}.  Rows: {Reconstruction, Interpolations, Axis walks}", fontsize=40,
                 fontweight='bold')
...
def least_gaussian_2d_subspace(model, real_embeddings, prior_samples):
    ...
    for _start in range(3):
        ...
        for _ in range(5000):
            ...
            step += 1
            if step % 10 == 0:
                loss_cur = model.sess.run(loss, feed_dict={real_embedding_var: real_embeddings,
                                                           prior_samples_var: prior_samples})
                rel_imp = abs(loss_cur - loss_prev) / abs(loss_prev)
                if rel_imp < 1e-2:
                    break
                loss_prev = loss_cur
        loss_final = model.sess.run(loss,
                                    feed_dict={real_embedding_var: real_embeddings, prior_samples_var: prior_samples})
...
def opts_check(model):
    ...
    assert opts['logvar-clipping'] is None or (
                len(opts['logvar-clipping']) == 2 and all([type(i) is int for i in opts['logvar-clipping']]))
    assert opts['z_prior'] in ['gaussian', 'uniform']
    assert opts['loss_reconstruction'] in ['bernoulli', 'L2_squared', 'L2_squared+adversarial',
                                           'L2_squared+adversarial+l2_filter', 'L2_squared+multilayer_conv_adv',
                                           'L2_squared+adversarial+l2_norm', 'patch_moments', 'normalised_conv_adv']
    if opts['loss_reconstruction'] in ['L2_squared+adversarial', 'L2_squared+adversarial+l2_filter',
        ...
        assert all(
            [opts['adv_cost_learning_rate_schedule'][i][1] < opts['adv_cost_learning_rate_schedule'][i + 1][1] for i in
             range(len(opts['adv_cost_learning_rate_schedule']) - 1)])
    assert opts['loss_regulariser'] in [None, 'VAE', 'beta_VAE',
                                        'WAE_MMD']  # either KL divergence of VAE or divergence of WAE
    ...
    if opts['optimizer'] == 'adam':
        ...
        assert all([opts['learning_rate_schedule'][i][1] < opts['learning_rate_schedule'][i + 1][1] for i in
                    range(len(opts['learning_rate_schedule']) - 1)])
